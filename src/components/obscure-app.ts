...
export class ObscureApp extends LitElement {
    ...
    render() {
        return html`
            ...
            ${!this.loaded ? html`<obscure-loader @loaded="${this._onLoaded}"></obscure-loader>` : ''}
            
            <main id="main-content" style="${!this.loaded ? 'visibility: hidden;' : ''}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="modal-content-col">
                            ...
                            <div class="modal-links">
                                <a href="#" target="_blank" class="link" id="link-gh">
                                    <span class="bracket">[</span> GITHUB <span class="bracket">]</span>
                                </a>
                                <a href="#" target="_blank" class="link" id="link-social">
                                    <span class="bracket">[</span> SOCIAL <span class="bracket">]</span>
                                </a>
                            </div>
                        </div>
                    </div>

                    <button class="modal-close interactable">[ESC] CLOSE_VIEW</button>
                </div>
            </div>
        `;
    }
    ...
    private _initMembers() {
        ...
        const revealBox = this.renderRoot.querySelector('.members-img-reveal') as HTMLElement;
        const revealImg = this.renderRoot.querySelector('#members-img') as HTMLImageElement;
    ...
    private _modalOpen(idx: number, imgUrl: string) {
        ...
        const wrapper = this.renderRoot.querySelector('.modal-wrapper') as HTMLElement;
        const container = this.renderRoot.querySelector('.modal-container') as HTMLElement;
        const name = this.renderRoot.querySelector('#modal-name') as HTMLElement;
        const role = this.renderRoot.querySelector('#modal-role') as HTMLElement;
        const bio = this.renderRoot.querySelector('#modal-bio') as HTMLElement;
        const img = this.renderRoot.querySelector('#modal-img') as HTMLImageElement;
        const gh = this.renderRoot.querySelector('#link-gh') as HTMLAnchorElement;
        const soc = this.renderRoot.querySelector('#link-social') as HTMLAnchorElement;
        ...
        tl.to(wrapper, { duration: 0.3, opacity: 1 })
          .to(container, { duration: 0.5, y: 0, opacity: 1, ease: "power4.out" }, "-=0.2");
    ...
    private _modalClose() {
        ...
        gsap.to(container, { duration: 0.3, y: 50, opacity: 0, ease: "power2.in" });
        gsap.to(wrapper, {
    ...
    private _initCursor() {
        ...
        const outline = this.renderRoot.querySelector('.cursor-outline') as HTMLElement;
        ...
        const hoverables = this.renderRoot.querySelectorAll('a, button, .project-item, .member-row');
